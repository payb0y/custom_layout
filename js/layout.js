...
(function () {
	...
	/** Grouping by app id. Apps not listed land in OTHER (rendered last). */
	const GROUPS = [
		{
			title: 'MAIN',
			apps: ['dashboard', 'spreed', 'talk'],
		},
		{
			title: 'COLLABORATION',
			apps: ['files', 'photos', 'gallery', 'activity', 'mail', 'contacts', 'calendar', 'notes'],
		},
		{
			title: 'PROJECT MANAGEMENT',
			apps: ['deck', 'tasks', 'forms', 'collectives', 'projects', 'organizational_chart', 'scrum', 'workflowengine'],
		},
		{
			title: 'ADMINISTRATION',
			apps: ['settings', 'admin_audit', 'logreader', 'serverinfo', 'admin', 'user_saml', 'theming'],
		},
	];
	...
	function getAppLabel(link) {
		const span = link.querySelector('.app-menu-entry__label, .app-menu-entry-label, span:not([class*="icon"])');
	...
	function buildIconNode(link) {
		...
		if (id) {
			...
			img.setAttribute('src', '/apps/' + encodeURIComponent(id) + '/img/app.svg');
			img.setAttribute('alt', '');
			img.setAttribute('aria-hidden', 'true');
			return img;
		}
	...
	function partitionByGroup(links) {
		...
		links.forEach((link) => {
			...
			if (id && !byApp.has(id)) byApp.set(id, link);
			else if (!id) byApp.set('__anon_' + byApp.size, link);
		});
		...
		GROUPS.forEach((g) => {
			...
			g.apps.forEach((appId) => {
				if (byApp.has(appId)) {
					groupLinks.push(byApp.get(appId));
					used.add(appId);
				}
			});
		...
		const leftover = [];
		byApp.forEach((link, id) => {
			if (!used.has(id)) leftover.push(link);
		});
	...
	function rebuildSidebar() {
		...
		if (groups.length === 0 || (groups.length === 1 && groups[0].title === 'OTHER')) {
